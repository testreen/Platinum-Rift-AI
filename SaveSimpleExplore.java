...
class Player {
    ...
    public static void updateFields(HashMap<Integer, Tile> tiles, int zoneCount, int myId){
        float DECAY_HQ = 0.8f;
        float DECAY_MY_UNITS = 0f;
        float DECAY_ENEMY_UNITS = 0.6f;
        float DECAY_PLATINUM = 3.6f;//0.3f;//0.9f;
        float DECAY_OWNER = 0.0f;
        float DECAY_UNEXPLORED = 2.9f;
...
class Explore extends Node {
    ...
    public String run(Tile unit) {
        ...
        closeTiles.put(GameState.tiles.get(i).total_score,i);

        for(int l=0; l < list.size(); l++){
            // if new best tile to move to
            closeTiles.put(GameState.tiles.get(list.get(l)).total_score,list.get(l));
...
class ExploreSave extends Node {
    @java.lang.Override
    public String run(Tile unit) {
        System.err.println("Exploring tile"+Integer.toString(unit.id));
        int i =unit.id;
